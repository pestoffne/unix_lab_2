...
static void handle_write(int signal, siginfo_t *siginfo, void *context) {
    ...
    read_avaible(pfd[index][0], index, log_fd);
    fprintf(stderr, "e hw %d\n", signal); // DEBUG
}
...
/*
    int flags;
    flags = fcntl(pfd[1][0], F_GETFL, 0); // TODO handle error? see man
    if (fcntl(pfd[1][0], F_SETFL, flags | O_NONBLOCK)) {
        perror("fcntl");
        exit(2);
    }
    flags = fcntl(pfd[2][0], F_GETFL, 0); // TODO handle error? see man
    if (fcntl(pfd[2][0], F_SETFL, flags | O_NONBLOCK)) {
        perror("fcntl");
        exit(2);
    }
*/

    if (fcntl(pfd[1][0], F_SETFL, O_NONBLOCK)) {
        perror("fcntl");
        exit(2);
    }
    if (fcntl(pfd[2][0], F_SETFL, O_NONBLOCK)) {
        perror("fcntl");
        exit(2);
    }
...
/*
        ...
        if (fcntl(pfd[2][0], F_SETFL, O_ASYNC)) {
            perror("fcntl F_SETFL ");
            exit(2);
        }
...
*/
        ...
        fprintf(stderr, "pfd[1][0] = %d\n", pfd[1][0]); // DEBUG
        fprintf(stderr, "pfd[1][1] = %d\n", pfd[1][1]); // DEBUG
        fprintf(stderr, "pfd[2][0] = %d\n", pfd[2][0]); // DEBUG
        fprintf(stderr, "pfd[2][1] = %d\n", pfd[2][1]); // DEBUG
